...
1. Hybrid CNN-ML models are significantly more effective than standalone CNNs for tea leaf disease classification. By leveraging a CNN backbone (like **ResNet-50**) solely for feature extraction and an ML algorithm (like **SVM**) for decision-making, the models achieved a more optimal "task separation." This allowed for robust classification and better generalization, particularly in multiclass scenarios where standalone CNNs struggled with complex feature localization.
2. Through Grad-CAM analysis, the research identified that while standalone CNNs can effectively localize disease symptoms, they are highly susceptible to "attention diversion". Common causes of error included focusing on background noise or confusing overlapping symptoms between diseases like Brown and Grey Blight. Furthermore, while image augmentation helped standalone models, it often introduced unnecessary noise for hybrids, suggesting that hybrids are more data-efficient and perform better on original images.
3. The **ResNet-50-SVM** model trained on original images emerged as the top-performing solution. Its success is attributed to the "skip connection" mechanism of **ResNet-50**, which prevents vanishing gradients and captures faint, detailed features, combined with the SVM’s ability to construct high-margin hyperplanes. This combination proved to be the most reliable and computationally efficient method for both binary and multiclass disease detection tasks

## Limitation
 * **Computational Resource Constraints**: The research was conducted using the free tier of Google Colab, which meant working within the specific hardware limits of the NVIDIA T4 GPU. These constraints, including session time limits and memory overhead, prevented the use of more resource-intensive, heavy-weighted models or extremely deep training cycles.
 * **Restricted Training Depth**: Due to the hardware limitations, the study could not implement a high number of training epochs or generate a massive scale of augmented data. This required a strategic trade-off to ensure the project remained within the "active session" windows provided by the cloud environment.
 * **Optimization for Efficiency**: To overcome these hurdles, the research focused on selecting light-weighted CNN architectures. By reducing the volume of augmented images and the epoch count, the study successfully prioritized training speed and computational efficiency without compromising the high performance of the final models.